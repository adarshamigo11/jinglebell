...
$user = requireUser();
$currentPage = 'commodity-detail';
$db   = getDB();

$stockId = (int)($_GET['id'] ?? 0);
if (!$stockId) { header('Location: commodity-market.php'); exit; }
...
$stmt = $db->prepare("
    ...
    WHERE s.id = ? AND s.is_active = 1 AND s.sector = 'Commodity'
");
$stmt->execute([$stockId]);
$stock = $stmt->fetch();
if (!$stock) { header('Location: commodity-market.php'); exit; }

// User's holding for this commodity
...
// Recent user orders for this commodity
...
<title><?= htmlspecialchars($stock['name']) ?> — TradeZenfy</title>
...
<style>
/* Time Range Buttons */
.time-btn { padding: 8px 16px; border: none; background: transparent; color: var(--groww-text-secondary); font-size: 13px; font-weight: 600; border-radius: 6px; cursor: pointer; transition: all 0.2s; }
    ...
    <div class="hero-top">
      <div>
        ...
        <div class="hero-badges">
          ...
          <span class="badge-sm"><?= htmlspecialchars($stock['industry'] ?? 'Commodity') ?></span>
        </div>
      </div>
    ...
    <div>
      ...
      <div class="card" style="margin-bottom: 24px;">
        ...
        <div style="padding: 12px 24px; border-bottom: 1px solid var(--groww-border); display: flex; gap: 8px;" id="time-range-selector">
          ...
          <button class="time-btn active" data-range="1M" onclick="loadChartData('1M')">1M</button>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 16px; padding: 16px 24px; border-top: 1px solid var(--groww-border); background: var(--groww-hover);" id="ohlc-stats">
          <div><div style="font-size: 12px; color: var(--groww-text-secondary); margin-bottom: 4px;">Open</div><div style="font-size: 14px; font-weight: 600;" id="stat-open">-</div></div>
          <div><div style="font-size: 12px; color: var(--groww-text-secondary); margin-bottom: 4px;">High</div><div style="font-size: 14px; font-weight: 600;" id="stat-high">-</div></div>
          <div><div style="font-size: 12px; color: var(--groww-text-secondary); margin-bottom: 4px;">Low</div><div style="font-size: 14px; font-weight: 600;" id="stat-low">-</div></div>
          <div><div style="font-size: 12px; color: var(--groww-text-secondary); margin-bottom: 4px;">Prev Close</div><div style="font-size: 14px; font-weight: 600;" id="stat-prevclose">₹<?= number_format((float)$stock['previous_close'], 2) ?></div></div>
        </div>
      </div>

      <!-- My Orders -->
      <div class="card">
        <div class="card-header"><i class="fa fa-list-alt" style="color:#6366f1;margin-right:8px"></i>My Orders for <?= htmlspecialchars($stock['name']) ?></div>
        <table>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td><?php if ($o['status'] === 'PENDING'): ?><button class="cancel-btn" onclick="cancelOrder(<?= $o['id'] ?>)">Cancel</button><?php endif; ?></td>
            </tr>
            <?php endforeach; ?>
            <?php if (empty($myOrders)): ?>
              <tr><td colspan="7" class="empty">No orders yet for this commodity.</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
        ...
        <div class="card-body" style="padding:20px">
          ...
          <div class="form-group">
            ...
            <div class="input-wrap">
              ...
              <input type="number" id="orderPrice" value="<?= $ltp ?>" step="0.01" min="0.01">
            </div>
          </div>
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => loadChartData('1M'));
} else {
  loadChartData('1M');
}
...
async function loadChartData(range) {
  ...
  const url = 'api/yahoo-chart-proxy.php?symbol=' + encodeURIComponent(yahooSymbol) + '&period1=' + period1 + '&period2=' + period2 + '&interval=' + interval;
  ...
  try {
    ...
    document.getElementById('chart-date').textContent = lastDate.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) + ' IST';
    
    const open = quotes.open[quotes.open.length - 1];
    const high = Math.max(...quotes.high.filter(h => h !== null));
    const low = Math.min(...quotes.low.filter(l => l !== null));
    
    document.getElementById('stat-open').textContent = '₹' + (open || 0).toFixed(2);
    document.getElementById('stat-high').textContent = '₹' + high.toFixed(2);
    document.getElementById('stat-low').textContent = '₹' + low.toFixed(2);
    
    const chartData = timestamps.map((ts, i) => ({ x: new Date(ts * 1000), y: quotes.close[i] })).filter(d => d.y !== null);
...
function renderChart(data, isPositive) {
  ...
  for (let i = 0; i < data.length; i += step) {
    ...
    const date = data[i].x;
    let label;
    if (currentRange === '1D' || currentRange === '5D') label = date.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' });
    else if (currentRange === '1M') label = date.toLocaleDateString('en-IN', { day: 'numeric', month: 'short' });
    else if (currentRange === '6M' || currentRange === '1Y') label = date.toLocaleDateString('en-IN', { month: 'short', year: '2-digit' });
    else label = date.toLocaleDateString('en-IN', { year: 'numeric' });
